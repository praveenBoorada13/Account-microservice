...
public class AccountController { 
	...
	public ResponseEntity<?> deleteAccount(@RequestHeader("Authorization") String token, @PathVariable String customerId) {

		accountService.hasEmployeePermission(token);

		List<Account> checkAccountExists = null;
		checkAccountExists = accountService.getCustomerAccount(token, customerId);
		if (checkAccountExists.size()==0) {
			System.out.println("ID DOESNOT EXIST");
			return new ResponseEntity<>("Customer Userid DOES NOT EXISTS", HttpStatus.NOT_ACCEPTABLE);
		}
		else
		{
		for (Account account : checkAccountExists) {
	...
	public ResponseEntity<List<Account>> getCustomerAccount(@RequestHeader("Authorization") String token,@PathVariable String customerId) {
		...
		return new ResponseEntity<>(accountService.getCustomerAccount(token, customerId), HttpStatus.OK);
	}
	...
	public ResponseEntity<List<String>> getAccountStatement(@RequestHeader("Authorization") String token) throws ParseException, WrongDateFormatException, WrongDateProvidedException{
		...
		return new ResponseEntity<>(accountService.getAccountStatement(), HttpStatus.OK);
	}
	...
	public ResponseEntity<List<String>> getAccountStatement(@RequestHeader("Authorization") String token,@PathVariable String from, @PathVariable String to) throws com.cognizant.accountservice.exceptionhandling.WrongDateFormatException, com.cognizant.accountservice.exceptionhandling.WrongDateProvidedException, ParseException{
		accountService.hasPermission(token);
		LOGGER.info("Account Statement List Returned");
		return new ResponseEntity<>(accountService.getAccountStatement(from,to), HttpStatus.OK);
	}
